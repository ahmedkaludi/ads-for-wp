<?php class adsforwp_pointers {
	...
	function adsforwp_admin_enqueue_scripts () {           
		$dismissed = explode (',', get_user_meta (wp_get_current_user ()->ID, 'dismissed_wp_pointers', true));                
		$do_tour = !in_array ('adsforwp_subscribe_pointer', $dismissed);                   
		if ($do_tour) {
			wp_enqueue_style ('wp-pointer');
			wp_enqueue_script ('wp-pointer');
			add_action('admin_print_footer_scripts', array($this, 'adsforwp_admin_print_footer_scripts'));
			add_action('admin_head', array($this, 'adsforwp_admin_head'));  // Hook to admin head
		}
	}
	function adsforwp_admin_head () {           
		?>
				...
				setup = function () {
					...
					<?php if ($button2) { ?>
						...
						jQuery ('#pointer-close').click (function () {
							$.post (ajaxurl, {
								pointer: 'aadsforwp_subscribe_pointer',
								action: 'dismiss-wp-pointer'
							});
						})
					<?php } ?>
