	...
	public function adsforwp_field_generator( $post ) {
           ...
           foreach ( $this->meta_fields as $meta_field ) {
			...
			switch ( $meta_field['type'] ) {
				case 'checkbox':                                   
                    $input = sprintf('<input %s id="%s" name="%s" type="checkbox" value="1">',
                    ...
                    if($meta_value){
                    	...
                    	foreach($meta_value as $key => $val){
                			...
                			if(!$expiry_status){
                				...
                				$expiry_msg = '<p class="afw-error">'.esc_html__('This ad has been expired on this date '.$ad_expire_to.'. Remove', 'ads-for-wp').'</p>';
                			}
							if(get_post_status($key) == 'publish'){
							  $input .= '<tr class="afw-group-add-ad-list" name="adsforwp_ads['.esc_attr($key).']">'
							    . '<td>'.esc_html($val).'<input type="hidden" name="adsforwp_ads['.esc_attr($key).']" value="'.esc_attr($val).'">'.esc_html($expiry_msg).'</td>' 
					...
					default:
						switch ($meta_field['id']) {
						    case 'adsforwp_group_ref_interval_sec':
						        $input = sprintf('<input class="afw_input" %s id="%s" name="%s" type="%s" value="%s"> '.esc_html__('milliseconds', 'ads-for-wp')
