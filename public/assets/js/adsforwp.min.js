...
function adsforwp_get_adtype(adType){
    ...
    var whereToDisplayVal = $("#wheretodisplay option:selected").val();
    ...
    for (var key in adtype_metafields) {
        ...
        if(obj.metaboxes){
            $.each(obj.metaboxes, function(key,val){
                if(adTypeVal == key && adTypeName == obj.id){
...
function adsforwp_get_display_metafields(adType){
    ...
    var displayTypeVal = $("#select_adtype").val();
    $(".adsforwp-advance-option-click").hide();
    if( adTypeName == 'adposition' && adTypeVal == 'number_of_paragraph'){
        $(".adsforwp-advance-option-click").show();
    }else{
        if(adTypeName == 'adposition' && adTypeVal == '50_of_the_content'){
            $(".adsforwp-advance-option-click").hide();    
        }
    }
    if(adTypeName == 'wheretodisplay' && adTypeVal != 'sticky'){
        if(displayTypeVal =='ad_image' || displayTypeVal == 'doubleclick'){
            ...
            $("#"+adTypeName+" option[value='sticky']").remove();    
        }
    }
